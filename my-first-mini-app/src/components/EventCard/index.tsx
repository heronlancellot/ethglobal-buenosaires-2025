...
export const EventCard = ({
  ...
  return (
    <div 
      className="flex gap-4 mb-5 bg-white cursor-pointer"
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-between min-w-0">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[11px] text-[#757683] truncate">Offered by {organizer}</span>
        </div>

        {location && (
          <div className="flex items-center gap-1 mb-2">
            <Pin className="w-3.5 h-3.5 text-[#1f1f1f]" strokeWidth={2} />
            <span className="text-[11px] text-[#757683]">{location}</span>
          </div>
        )}

        <div className="flex items-center justify-between gap-2 mt-2">
          <span className="font-bold text-[#1f1f1f] text-[15px]">{price}</span>
          <button
            onClick={(e) => {
              e.stopPropagation();
              onJoin?.(id);
